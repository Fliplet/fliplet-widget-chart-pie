...
Fliplet.Widget.instance('chart-pie-1-1-0', function(data) {
  ...
  function init() {
          ...
          return Fliplet.Hooks.run('afterQueryChart', result)
            ...
            .then(function() {
              ...
              switch (data.dataSourceQuery.selectedModeIdx) {
                  ...
                  result.dataSourceEntries.forEach(function(row, i) {
                    data.entries.push({
                      name:
                        ...
                        T('widgets.charts.pie.category', { count: i + 1 }),
                      y:
                ...
                case 1:
                  // Summarize data
                  data.name = T('widgets.charts.pie.title', {
    ...
    function getThemeColor(colorKey) {
      return themeValues && themeValues.hasOwnProperty(colorKey)
        ? themeValues[colorKey]
    ...
    function getColor(key, device) {
      if (!device) {
        return (
          themeValues && themeValues.hasOwnProperty(key) && themeValues[key]
        );
      }
